        ...
        <div className="h-[100dvh] w-screen text-neutral-100 overflow-hidden bg-neutral-900 relative">
            ...
            {isStreaming && (
                <>
                    ...
                    <div
                        className="hidden md:flex absolute bottom-4 left-1/2 -translate-x-1/2 items-center space-x-3 bg-neutral-900/30 backdrop-blur-xl p-2 rounded-3xl border border-neutral-100/10 shadow-lg z-10">
                        ...
                        <ControlButton onClick={handleFlipCamera}
                        ...
                        <ControlButton onClick={() => setIsMirrored((m) => {
                            ...
                            if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
                                socketRef.current.send(JSON.stringify({
                                    event: "video-mirror",
                                    data: {streamId: streamerId, mirrored: newMirrored}
                                }));
                            }
                            return newMirrored;
                        })}
                            ...
                            setVideoRotation((r) => {
                                ...
                                if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
                                    socketRef.current.send(JSON.stringify({
                                        ...
                                        data: {streamId: streamerId, rotation: newRotation}
                                    }));
                                }
                                return newRotation;
                            });
                        ...
                        {/* Pause/Stop Controls */}
                        <ControlButton onClick={handlePauseStream}
                    ...
                    {/* Mobile Controls */}
                    <div
                        className="md:hidden absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center z-20 space-y-2">
                        {/* Row 1: Camera/Mic/Mirror/Rotate Controls */}
                        <div
                            className="flex items-center justify-center space-x-3 bg-neutral-900/30 backdrop-blur-xl p-2 rounded-3xl border border-neutral-100/10 shadow-lg max-w-xs">
                            <ControlButton onClick={toggleMute}
                                           className={isMuted ? "bg-red-500/80 hover:bg-red-500 text-white" : "bg-neutral-800/70 hover:bg-neutral-700/90 text-neutral-200"}>
                                {isMuted ? <MicOff className="w-6 h-6"/> : <Mic className="w-6 h-6"/>}
                            </ControlButton>
                        ...
                        <div className="flex justify-between w-screen px-7">
                            <div
                                className="flex items-center space-x-3 bg-neutral-900/30 backdrop-blur-xl p-2 rounded-3xl border border-neutral-100/10 shadow-lg max-w-xs">
                                <ControlButton onClick={handlePauseStream}
                                               className={isPaused ? "bg-teal-500/80 hover:bg-teal-500 text-white" : "bg-yellow-500/80 hover:bg-yellow-500 text-neutral-900"}>
                                    {isPaused ? <Play className="w-6 h-6"/> : <Pause className="w-6 h-6"/>}
                                </ControlButton>
                                <ControlButton onClick={handleStopStream}
                                               className="bg-red-500/80 hover:bg-red-500 text-white">
                                    <Square className="w-6 h-6"/>
                                </ControlButton>
                            </div>
                            <div
                                className="flex items-center bg-neutral-900/30 backdrop-blur-xl p-2 rounded-3xl border border-neutral-100/10 shadow-lg ml-2">
                                <ControlButton onClick={() => setShowMobileOverlay(true)}
                                               className="bg-neutral-800/70 hover:bg-neutral-700/90 text-neutral-200">
                    ...
                    {(showMobileOverlay || mobileOverlayVisible) && (
                        ...
                        >
                            <button onClick={() => setShowMobileOverlay(false)}
                                    className="mb-4 p-2 rounded-full bg-neutral-800/80 text-white text-xl self-center">
                            ...
                            <div className="w-full max-w-md mx-auto flex flex-col items-center space-y-4">
                                <div className="bg-neutral-900/80 border border-neutral-100/10 p-4 rounded-2xl w-full">
                                    ...
                                    <div className="space-y-3 text-sm">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span
                                                className={`font-semibold px-2 py-0.5 rounded-md text-xs ${isStreaming ? isPaused ? "bg-yellow-500/20 text-yellow-300" : "bg-red-500/20 text-red-400" : "bg-neutral-700 text-neutral-300"}`}>
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span
                                                className="capitalize">{currentCamera === "user" ? "Front" : "Back"}</span>
                                        </div>
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-2 mt-1">
                                                {streamTags?.length ? (
                                                    streamTags.map((tag, i) => (
                                                        <span key={i}
                                                              className="bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full text-xs">{tag}</span>
                                                    ))
                                                ) : (
                                ...
                                <div className="w-full flex-1 max-h-[40vh] min-h-[180px] overflow-y-auto">
                                    <Chat streamId={streamId} username={username} userId={streamerId}
                                          socket={socketRef.current} myStream={true}/>
                                </div>
                            </div>
                            <style>{`
