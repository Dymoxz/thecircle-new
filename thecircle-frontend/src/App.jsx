...
const HomePage = () => {
  ...
  // New states for dynamic streams
  const [streams, setStreams] = useState([]);
  const [loadingStreams, setLoadingStreams] = useState(true);
  const [streamError, setStreamError] = useState(null);

  const socketRef = useRef(null); // Ref for WebSocket

  useEffect(() => {
    document.title = "The Circle - Home";
  }, []);

  // Effect for fetching current user's subscriptions
  useEffect(() => {
  ...
  }, [navigate]); // Only re-run if navigate object changes

  // Effect for fetching live streams via WebSocket (logic from ViewerPage)
  useEffect(() => {
    ...
    socket.onopen = () => {
      ...
      setLoadingStreams(false); // Connection is open, so stop initial loading
    };
    ...
    socket.onmessage = (event) => {
      ...
      switch (msg.event) {
        case 'streams':
          // Update streams state with the received list
        ...
        case 'stream-started':
        case 'stream-ended':
          // Refresh the stream list when a stream starts or ends
          socket.send(JSON.stringify({ event: 'get-streams', data: {} }));
          break;
        case 'error':
 ...
 /* const filteredStreams = streams.filter(stream =>
      ...
      stream.streamerUserName.toLowerCase().includes(searchTerm.toLowerCase()) // Changed from 'streamer' to 'streamerUserName'
  );*/
  ...
  // Function to handle clicking a live stream card
  const handleStreamClick = (streamId) => {
    navigate(`/viewer?streamId=${streamId}`); // Navigate to ViewerPage with streamId
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 flex-grow mt-12">
            {loadingStreams ? (
                ...
                filteredStreams.map(stream => (
                    <div
                        key={stream.streamId} // Use streamId from dynamic data
                        ...
                        onClick={() => handleStreamClick(stream.streamId)} // Pass streamId to handler
                    >
                      ...
                      <div className="p-4">
                        ...
                        <p className="text-gray-800 text-sm mb-2 truncate">{stream.streamId}</p> {/* Use streamerUserName */}
                        <div className="flex items-center text-sm text-neutral-600">
                          <Eye className="w-4 h-4 mr-1" /> {stream.streamId || 0} viewers {/* Use viewerCount */}
                        </div>
                      </div>
                    </div>
                ))
