...
import {ArrowDownWideNarrow, Camera, Eye, Menu, Search, SlidersHorizontal, User, X,} from "lucide-react";
...
const HomePage = () => {
    ...
    return (
        <div
            className="min-h-screen bg-gradient-to-br bg-gradient-to-br from-[#7a1a1a] via-[#800000] to-[#2d0a14] text-white flex p-4 font-oswald overflow-hidden relative">
            {/* Hamburger for mobile */}
            <button
                className="fixed z-40 top-6 right-6 md:hidden bg-[#800000] text-white rounded-full p-2 shadow-lg focus:outline-none"
            ...
            {/* Close button for mobile, outside the panels */}
            {sidebarOpen && (
                <button
                    className="fixed top-4 right-4 z-50 md:hidden bg-[#800000] text-white rounded-full p-1.5 shadow-lg"
                    onClick={() => setSidebarOpen(false)}
            ...
            <div className="flex-grow flex flex-col p-4">
                <div className="flex flex-col md:flex-row items-center justify-between mb-8">
                    ...
                    <div className="flex-grow max-w-xl md:mx-8 w-full relative">
                        ...
                        {showUserDropdown && (
                            <div
                                className="absolute z-50 left-0 right-0 mt-2 bg-white/95 rounded-xl shadow-xl max-h-60 overflow-y-auto border border-white/30">
                                {searchTerm.length < 3 ? (
                                    ...
                                    userResults.map((user) => (
                                        ...
                                        >
                                            <div
                                                className="w-8 h-8 rounded-full bg-neutral-700 flex items-center justify-center mr-3 text-white font-bold">
                    ...
                    <div className="flex space-x-2 mt-4 md:mt-0">
                        ...
                        <div className="relative" ref={filterRef}>
                            ...
                            {filterOpen && (
                                <div
                                    className="absolute right-0 mt-2 w-64 bg-white/90 backdrop-blur-lg rounded-lg shadow-xl z-50 p-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6  mt-12">
                    {loadingStreams ? (
                        <div className="col-span-full text-center text-white text-lg py-10">
                            <div
                                className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-white mx-auto mb-4"></div>
            ...
            <div
                className={`
                    fixed inset-0 z-40 flex items-start justify-center md:hidden
                    ${sidebarOpen ? "flex" : "hidden"}
                `}
                style={{
                    pointerEvents: sidebarOpen ? "auto" : "none",
                    background: "rgba(20, 0, 0, 0.92)", // much darker overlay
                }}
            >
                <div
                    className={`
                        flex flex-col space-y-4 w-full max-w-xs mx-auto
                    `}
                    style={{
                        marginTop: '10vh', // start about 1/5th from the top
                    }}
                >
                    {/* Profile panel */}
                    <div
                        className={`
                            bg-white border-white/10 rounded-3xl p-4 shadow-md shadow-black/30 flex flex-col items-center max-h-[40vh] overflow-y-auto
                        `}
                    >
                        ...
                        >
                            <div
                                className="w-10 h-10 rounded-full bg-neutral-700 flex items-center justify-center mr-2 border-2 border-[#800000] overflow-hidden">
                    ...
                    <div
                        className={`
                            bg-white border-white/10 rounded-3xl p-8 shadow-md shadow-black/30 flex flex-col max-h-[50vh] overflow-y-auto
                        `}
                    >
                        ...
                        {loadingSubscriptions ? (
                            <div className="flex justify-center items-center h-20">
                                <div
                                    className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-teal-500"></div>
                            </div>
                        ) : subscriptionError ? (
                                ...
                                {subscriptions.map(
                                    (sub) =>
                                        sub?.user && (
                                            ...
                                            >
                                                <div
                                                    className="w-6 h-6 rounded-full mr-3 flex-shrink-0 overflow-hidden bg-neutral-700 flex items-center justify-center text-white text-xs font-bold">
                                                    {sub.user?.userName
                                                        ?.charAt(0)
                                                        .toUpperCase() || "U"}
                                                </div>
            ...
            <div className="hidden md:flex w-80 flex-shrink-0 ml-6 mt-0 flex-col space-y-6">
                {/* Profile panel */}
                <div
                    className="bg-white/80 border-white/10 rounded-3xl p-4 shadow-md shadow-black/30 flex flex-col items-center">
                    {/* Profiel sectie - aparte klik handler */}
                    <div
                        className="flex items-center justify-start w-full mb-2 cursor-pointer"
                        onClick={(e) => {
                ...
                {/* Subscriptions panel */}
                <div
                    className="flex-grow bg-white/80 border-white/10 rounded-3xl p-8 shadow-md shadow-black/30 overflow-y-auto">
                    <h3 className="text-lg font-bold text-gray-800 mb-4">
                        Subscriptions
                    </h3>
                    {loadingSubscriptions ? (
