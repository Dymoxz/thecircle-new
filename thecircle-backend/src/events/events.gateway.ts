...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleDisconnect(client: WebSocket) {
    ...
    if (type === 'streamer' && streamId) {
      ...
      if (stream) {
        ...
        console.log(
          `[CLEANUP] Streamer ${id} disconnected, streamId=${streamId} removed`,
        );
      }
      ...
      if (stream) {
        ...
        console.log(
          `[CLEANUP] Viewer ${id} disconnected from streamId=${streamId}`,
        );
      }
    }
  ...
  handleGetStreams(@ConnectedSocket() socket: WebSocket) {
    ...
    const activeStreams = Array.from(this.streams.entries())
      .filter(
        ([, streamData]) =>
          streamData.streamerSocket.readyState === WebSocket.OPEN,
      )
  ...
  handleChatMessage(
    ...
    const recipientIds = [stream.streamerId, ...Array.from(stream.viewers)];
